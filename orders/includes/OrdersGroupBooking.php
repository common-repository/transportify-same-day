...
/**
 * OrdersGroupBooking class that will display our custom table
 * records in nice table
 */

class OrdersGroupBooking extends WP_List_Table
{
    ...
    public $user_type;
    public $currency;
    protected $group_booking_ids;
    protected $except_ids;

    /**
     * OrdersGroupBooking constructor.
     */
    function __construct($group_booking_ids = [], $except_ids = [])
    {
        ...
        $this->group_booking_ids = (empty($group_booking_ids)) ? [0] : $group_booking_ids;
     ...
     * @return HTML
     */
    function column_sort_group_booking($item)
    {
        return '<div class="warraper-arrow-sort" data-booking-id="' . $item['id'] . '"></div>';
    }
    ...
    {
        $options[] = '<a data-id="' . $item['id'] . '" data-order-id="' . $item['order_id'] . '"   class="remove-from-list remove-from-list-js dropdown-item ">Remove from list</a>';
        $more_button = "<div data-data='" . json_encode($item) . "' class='get_order_data_" . $item["id"] . " btn-group button-dropdown'><button type='button' class='button'>More</button><div class='dropdown-menu dropdown-menu-right'>" . implode('', $options) . "</div></div>";
        $action =  $more_button;
        return $action;
    }
...
<?php
    ...
    function prepare_items()
    {
        try {
            ...
            $in_ids = implode(' , ', $this->group_booking_ids);
            $where[] = 'id IN ( ' . $in_ids . ' )';
            ...
            if ($where) {
                ...
                if (!empty($this->group_booking_ids)) {
                    $orderby = 'FIELD (id, ' . $in_ids . ' )';
                }
